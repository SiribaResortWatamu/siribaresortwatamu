 ...
 * Brand marks.
 *
 * lucide-react dropped its brand icons, and a generic speech bubble does not
 * read as "WhatsApp" — which matters, because WhatsApp is how most guests on
 * this coast actually get in touch. These are the official glyphs, drawn as
 * solid paths so they sit correctly next to lucide's stroked icons.
 *
 * No hooks here, so they work in server and client components alike.
 */
...
export function WhatsAppIcon({ size = 18, className, title }: IconProps) {
  return (
...
export function FacebookIcon({ size = 18, className, title }: IconProps) {
  return (
    <svg {...base(size, className, title)}>
      {title && <title>{title}</title>}
